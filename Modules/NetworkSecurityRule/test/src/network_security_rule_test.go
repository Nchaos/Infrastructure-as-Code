...
func TestNetworkSecurityGroup(t *testing.T) {
	...
	resourceGroupIdMap := terraform.Output(t, terraformOptions, "nsg_ids_map")

	expectedResourceGroupIdMap := string("map[nsr-val-tf:/subscriptions/d0ce61fb-0b7a-4f0d-9ea2-b1ae691749e0/resourceGroups/rg-nsr-val-tf/providers/Microsoft.Network/networkSecurityGroups/nsr-val-tf]")

	// Verify we're getting back the outputs we expect
	require.Equal(t, expectedResourceGroupIdMap, resourceGroupIdMap)

	// Run `terraform output` to get the value of an output variable
	resourceGroupLocationMap := terraform.Output(t, terraformOptions, "resource_group_locations_map")

	expectedResourceGroupLocationMap := string("map[rg-nsr-val-tf:usgovarizona]")
	// Verify we're getting back the outputs we expect
